  ...
  """
    ...
    """
        ...
        with tf.variable_scope(scope,
            ...
            self.sequence_output = tf.cast(self.all_encoder_layers, tf.float32)
  ...
  """
    ...
    if use_token_type:
        ...
        if use_one_hot_embeddings:
            ...
            token_type_embeddings = tf.gather(token_type_table,
                                              flat_token_type_ids)
...
def attention_layer(from_tensor,
  ...
  """
    ...
    def ckpt_attention(from_t, to_t, attention_mask):

        from_shape = get_shape_list(from_t, expected_rank=[2, 3])
        to_shape = get_shape_list(to_t, expected_rank=[2, 3])
        ...
        from_tensor_2d = reshape_to_matrix(from_t)
        to_tensor_2d = reshape_to_matrix(to_t)
        ...
        # attention_scores = attention_scores_calculation(query_layer, key_layer)
    ...
    if ckpt:
        ckpt_attention = tf.contrib.layers.recompute_grad(ckpt_attention)
    context_layer = ckpt_attention(from_tensor, to_tensor, attention_mask)
    return context_layer
    ...
    # #   H = `size_per_head`
    # def attention_calculation(a_2d, b_2d):
    ...
    # # attention_calculation = tf.contrib.layers.recompute_grad(
    # #     attention_calculation)
    # key_layer, value_layer, query_layer = attention_calculation(
    #     from_tensor_2d, to_tensor)
