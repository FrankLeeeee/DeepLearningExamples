...
import gradient_checkpointing as gc

gc.MIN_CHECKPOINT_NODE_SIZE = 1024


def gradients_auto(ys, xs, grad_ys=None, **kwargs):
    return gc.gradients(ys, xs, grad_ys, checkpoints='memory', **kwargs)
...
def create_optimizer(loss,
    ...
    if hvd is not None and (num_accumulation_steps == 1 or
        ...
        optimizer = hvd.DistributedOptimizer(
            ...
            if use_fp16 or manual_fp16 else Compression.none)
    if use_fp16:
    ...
    # grads = gradients_auto(loss * 1.0 / num_accumulation_steps, tvars)
    ...
    if num_accumulation_steps > 1:
        ...
        ]) if use_fp16 or manual_fp16 else tf.constant(True, dtype=tf.bool)
