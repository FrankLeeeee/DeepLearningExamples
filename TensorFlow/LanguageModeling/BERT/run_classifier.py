...
flags.DEFINE_bool("grad_checkpointing", False,
                  "Enable gradient checkpointing to save memory usage")

flags.DEFINE_string("ckpt_mode", "none",
                    "mode for gradient checkpointing: none, memory and speed")
...
def get_frozen_tftrt_model(bert_config, shape, num_labels,
    ...
    tf_config.gpu_options.tf_config.gpu_options.allow_growth = 0.5
...
def main(_):
    ...
    # if FLAGS.grad_checkpointing and FLAGS.do_train and FLAGS.grad_checkpointing is not 'none':
    #   ckpt_fn = getattr(gc, 'gradients_{}'.format(FLAGS.ckpt_mode))
    #   tf.__dict__["gradients"] = ckpt_fn
